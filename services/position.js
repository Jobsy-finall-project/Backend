...
async function deletePosition(positionId, hrId) {
  ...
  if (position._doc.hrId !== hrId) {
    throw new Error(
      "cant delete this position because it wasn't create by you"
    );
  }
  ...
  const updated_positions = current_company._doc.positions.filter(
    cur => String(cur) !== positionId
  );
  const updated_company = await Company.findByIdAndUpdate(companyId, {
